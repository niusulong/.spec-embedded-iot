...
> ⚠️ **判块状态硬规则（防 double-free 被误判为越界写）**：解读 `osMemTrace_t` 字段（magic/size/user/...）之前，**必须先确认受害块是在用还是 FREE** —— 看下一块的 `PREV_INUSE` 位 + 空闲链表双向回指，或直接信 `heap_walker.py` 的 inuse/free 判定。`dlfree` **不清除** `trace.magic`，已释放块的 magic 仍 = 0x1212（合法），**绝不能据 "magic 合法" 判块在用 / 排除 double-free**。`dlmalloc.c:2066`（check_mem_trace downflow）尤其要按此规则走（见下表 + `references/heap-corruption-guide.md`「OS_MEM_TRACE 与 dlmalloc.c:2066」节）。
...
| **2066** | `osFreeTrace`/`check_mem_trace` | **downflow**：块尾 down magic(`*down`/`*(down+1)`) != 0x89。down 位置用 `trace.size` 算（`down=trace+trace.size+16`），trace.size 被改即算飞。**两类根因**：① 受害块 FREE → **double-free**（trace.size 被第一次 free 写入的 fd/bk 覆盖成 bin 头地址）；② 受害块 INUSE → 越界写踩坏 trace 头。**先跑 `double_free_detect.py` 判块状态**，不要只看 magic（dlfree 不清 magic） |
...
│   │   ├── **2066 downflow + 受害块 FREE**（下一块 PREV_INUSE=0 + 空闲链表回指）→ ★ **double-free**（free 后未置空；跑 `double_free_detect.py`；查重复释放路径，如对象重复析构 / `free(ptr)` 后未 `ptr=NULL`）
│   │   └── 2066 downflow + 受害块 INUSE + trace.size 异常 → 越界写踩坏 trace 头（guard bytes / osMemTrace_t 加 CRC 定位）
