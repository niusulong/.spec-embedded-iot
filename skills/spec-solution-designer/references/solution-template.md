...
| **平台** | [纯平台名，如 EC626 / ASR1603；必填，禁止带架构后缀/括号/版本号；多平台用逗号分隔] |
| **模块** | [如 网络 / 蓝牙 / 文件系统 / 电源管理] |
| **方案概述** | [一句话说明技术实现思路] |
| **影响面** | [改动的层次/模块/协议链路，如：应用层+PPP拨号+TCP重建] |
| **检索关键词** | [5-8 个中英文关键词，逗号分隔，如：PPP,拨号,断连恢复,LWIP] |
| **关联需求文档** | [如 需求.md] |
...
## 9. 接口设计
### 9.1 对外接口/API
> 函数/AT 命令命名遵循 spec-neoway-coding-standards 命名规范。
...
### 9.2 配置项与宏控
> 新增功能宏控命名须遵循 Neoway 编码规范——功能宏 `FEATURE_NWY_[AT|OPEN]_FUNC_[FUNC1]_[BZ]`、客户定制宏 `FEATURE_NWY_CUS_[INDEX]`，宏名大写+下划线、不含项目名（详见 spec-neoway-coding-standards §3.4）。

| 宏控/配置项 | 类型 | 默认(开/关) | 生效范围（哪些项目/客户版本） | 说明 |
|--------|------|--------|--------|------|
| [如 FEATURE_NWY_AT_FTP] | 功能宏 | 开 | 基线所有 AT 版项目 | [用途] |
| [如 FEATURE_NWY_CUS_MT] | 客户定制宏 | 开 | 美团客户项目 | [定制说明] |
| [NV/运行时配置] | [类型] | [默认] | — | [用途] |
