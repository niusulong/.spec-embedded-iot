...
│   └── analysis/               # dump 分析脚本输出（run_all.py 归档：01_*.txt… + INDEX.md + _meta.json）
├── code-summary/{模块}/        # 代码总结输出
└── requirement/{项目ID}_{功能}/ # 需求.md / 方案.md / 计划.md 同目录并排

.claude-plugin/                 # Claude Code 插件描述（plugin.json + marketplace.json）
...
knowledge/                      # 跨项目持久化知识库（独立 git 仓库，主仓库 .gitignore 排除）
```
